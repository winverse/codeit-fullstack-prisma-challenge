...
commentsRouter.get('/post/:postId/details', async (req, res) => {
  try {
    const comments = await commentRepository.getCommentsWithDetails(req.params.postId);
    res.json(comments);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
commentsRouter.put('/:id', async (req, res) => {
  try {
    const updatedComment = await commentRepository.updateComment(req.params.id, req.body);
